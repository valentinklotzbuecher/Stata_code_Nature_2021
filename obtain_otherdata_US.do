...
* Mobility:
import delimited "$rawdata\Further Sources\Google Community Mobility Reports\Global_Mobility_Report.csv",  clear 
...
save "$rawdata\Further Sources\Google Community Mobility Reports\NAmerica_Mobility_daily.dta", replace
...
save "$rawdata\Further Sources\Google Community Mobility Reports\USA_Mobility_Report.dta", replace


* Canadian provinces population
import delimited "$rawdata\Further Sources\Oxford COVID-19 Government Response Tracker\canada_population.csv",  clear 
keep v1 v5
drop in 1/7
drop in 14/23
rename v1 state
...
save "$rawdata\Further Sources\Oxford COVID-19 Government Response Tracker\canada_population.dta", replace
...
* OI tracker
import delimited "$rawdata\Further Sources\Opportunity Insights Economic Tracker\GeoIDs - State.csv",  clear 

rename statename state
save "$rawdata\Further Sources\Opportunity Insights Economic Tracker\states.dta", replace



import delimited "$rawdata\Further Sources\Opportunity Insights Economic Tracker\UI Claims - State - Weekly.csv",  clear 
bysort statefips ( year month day_endofweek): gen nweek = _n + 51

save "$rawdata\Further Sources\Opportunity Insights Economic Tracker\UIclaims.dta", replace


// import delimited "$rawdata\Further Sources\Opportunity Insights Economic Tracker\Zearn - State - Weekly.csv",  clear 



// import delimited "$rawdata\Further Sources\Opportunity Insights Economic Tracker\Policy Milestones - State.csv",  clear 



import delimited "$rawdata\Further Sources\Opportunity Insights Economic Tracker\COVID - State - Daily.csv",  clear 
...
save "$rawdata\Further Sources\Opportunity Insights Economic Tracker\COVID.dta", replace




import delimited "$rawdata\Further Sources\Opportunity Insights Economic Tracker\Employment Combined - State - Daily.csv",  clear 
...
save "$rawdata\Further Sources\Opportunity Insights Economic Tracker\Employment.dta", replace
...
// * OxCGRT subnational data
// import delimited "$rawdata\Further Sources\Oxford COVID-19 Government Response Tracker\OxCGRT_latest.csv",  clear 

// keep if countryname == "United States Virgin Islands"| countryname == "Puerto Rico"|  countryname == "Guam"
// gen state = countryname
// replace state =  "Virgin Islands" if state == "United States Virgin Islands"

// replace regioncode = "US_VI" if state == "Virgin Islands"
// replace regioncode = "US_PR" if state == "Puerto Rico"
// replace regioncode = "US_GM" if state == "Guam"
// replace countryname = "United States" if state == "Virgin Islands"
// replace countryname = "United States" if state == "Puerto Rico"
// replace countryname = "United States" if state == "Guam"
// replace countrycode = "USA" if state == "Virgin Islands"
// replace countrycode = "USA" if state == "Puerto Rico"
// replace countrycode = "USA" if state == "Guam"

// save "$rawdata\Further Sources\Oxford COVID-19 Government Response Tracker\territories.dta", replace


import delimited "$rawdata\Further Sources\Oxford COVID-19 Government Response Tracker\covid-policy-tracker-master\data\OxCGRT_latest.csv",  clear 
...
merge m:1 state using "$rawdata\Further Sources\Oxford COVID-19 Government Response Tracker\canada_population.dta"

// append using "$rawdata\Further Sources\Oxford COVID-19 Government Response Tracker\territories.dta"
...
merge m:1 state using "$rawdata\Further Sources\Opportunity Insights Economic Tracker\states.dta"

replace state_pop2019 = popul if state_pop2019 == .
drop popul

merge m:1 statefips nweek using "$rawdata\Further Sources\Opportunity Insights Economic Tracker\UIclaims.dta", gen(_m2)

merge m:1 statefips nweek using "$rawdata\Further Sources\Opportunity Insights Economic Tracker\Employment.dta", gen(_mE)

merge m:1 statefips nweek using "$rawdata\Further Sources\Opportunity Insights Economic Tracker\COVID.dta", gen(_m3)
...
merge 1:1 state nweek using  "$rawdata\Further Sources\Google Community Mobility Reports\USA_Mobility_Report.dta", gen(_m4)
...
save "$rawdata\Further Sources\NorthAmerica_otherdata.dta", replace

erase "$rawdata\Further Sources\Oxford COVID-19 Government Response Tracker\canada_population.dta"
erase "$rawdata\Further Sources\Opportunity Insights Economic Tracker\Employment.dta"
erase "$rawdata\Further Sources\Opportunity Insights Economic Tracker\states.dta"
erase  "$rawdata\Further Sources\Opportunity Insights Economic Tracker\COVID.dta"
erase "$rawdata\Further Sources\Opportunity Insights Economic Tracker\UIclaims.dta"
